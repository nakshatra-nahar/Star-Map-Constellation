...
function draw_star(context, s) {
	...
	context.beginPath();
	context.arc(s.pos.x, s.pos.y, s.radius * 2.5, 0, 2 * Math.PI);
	context.closePath();
	context.fill();
}
...
function draw_sky(context, w, h) {
	...
	context.globalCompositeOperation = "source-over";
	// Remove background fill to maintain transparency
	// context.fillStyle = bgcolor;
	context.beginPath();
	context.arc(w / 2, h / 2, w / 2, 0, 2 * Math.PI);
	context.closePath();
	// context.fill();  // Comment this to prevent background fill, keeping transparency
	if (!clipped) {
		context.clip();
		clipped = true;
	}
	...
	for (i = 0; i < 9; i++) {
		...
		if (planet[i].pos.visible)
			draw_planet(context, planet[i]);  // Make sure draw_planet is correctly defined and used
	}
